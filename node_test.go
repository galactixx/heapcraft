...
func TestHeapNodeCreation(t *testing.T) {
	// Test CreateHeapPair
	heapNode := CreateHeapPair("test", 42)
	assert.Equal(t, "test", heapNode.Value())
	assert.Equal(t, 42, heapNode.Priority())

	// Test CreateHeapPairPtr
	heapNodePtr := CreateHeapPairPtr(123, 456.78)
...
func TestRadixPairCreation(t *testing.T) {
	...
	radixPair := CreateRadixPair(true, 3.14)
...
func TestSimpleNodeInterface(t *testing.T) {
	...
	// Test with HeapNode
	heapNode := CreateHeapPair("hello", 123)
...
func TestRadixPairGenericTypes(t *testing.T) {
	// Test RadixPair with different type combinations
	complexRadix := CreateRadixPair(map[string]int{"a": 1, "b": 2}, 99.9)
	assert.Equal(t, map[string]int{"a": 1, "b": 2}, complexRadix.Value())
	assert.Equal(t, 99.9, complexRadix.Priority())

	pointerRadix := CreateRadixPair(&[]int{1, 2, 3}, uint(123))
...
func TestNodeEquality(t *testing.T) {
	...
	node1 := CreateHeapPair("test", 42)
	node2 := CreateHeapPair("test", 42)
	...
	node3 := CreateHeapPair("different", 42)
...
func TestPointerVsValue(t *testing.T) {
	...
	valueNode := CreateHeapPair("test", 42)
	ptrNode := CreateHeapPairPtr("test", 42)
